...
import { Genre, MangaList } from 'types'
			...
			{!mangas || mangas.totalItems === 0 ? (
				...
				<div>
					...
					<Group position='center' mt='md'>
						<Pagination
							total={Math.ceil(mangas.totalItems / mangas.perPage)}
...
export const getServerSideProps = async ({
	...
	try {
		mangaRes = await client.records.getList('mangas', 1, 30, {
			expand: 'upload_by,view',
			sort,
			filter
		})

		genresRes = await client.records.getFullList('genres', 99999)
	} catch (error) {
		return {
			props: {
				mangas: null,
	...
	return {
		props: {
			mangas: {
				...mangas,
				items: mangas.items.map(serverDataTransform)
			},
			genres
		}
	}
