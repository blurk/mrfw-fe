...
const ChapterViewer = ({ data }: Props) => {
  ...
  const rows = useMemo(() => {
    const sorted = data.sort((item1, item2) => {
      ...
      return sortBy === 'old' ? date1.getTime() - date2.getTime() : date2.getTime() - date1.getTime();
    });
  ...
  return (
    <Paper p="sm" mt="md" withBorder>
      <Group position="apart">
        ...
        <Button onClick={() => toggleSortBy()} compact disabled={data.length === 0}>
      ...
      {data.length > 0 && (
        <Group grow pr="lg" mb="sm">
          ...
          <Center inline>
            ...
            {sortBy === 'old' ? <IconArrowUp size={18} /> : <IconArrowDown size={18} />}
          </Center>
        </Group>
      )}
      ...
      {data.length === 0 ? (
        <Stack justify="center" sx={{ height: 300 }}>
          ...
          />
          <Anchor align="center" component={Link} href={Routes.MANGA}>
            Đọc truyện khác
          </Anchor>
        </Stack>
      ) : (
