...
export function FilePreview({ file }: { file: File | string }) {
  ...
  return (
    ...
    >
      <Image src={imageUrl} alt="preview" layout="fill" objectFit="cover" objectPosition="center" />
    </Box>
  );
}
  ...
  if (Array.isArray(value)) {
    return (
      <ScrollArea type="auto" style={{ height: 300 }}>
        <SimpleGrid cols={4} breakpoints={[{ maxWidth: 'sm', cols: 1 }]} mt={value.length > 0 ? 'xl' : 0}>
