...
import { updatePassword, useSession } from 'utils'
...
const FormChangePassword = (props: Props) => {
	...
	const handleSubmit = async (data: ChangePasswordRequest) => {
		try {
			await client.users.confirmPasswordReset(
				data.token,
				data.password,
	...
	const sendEmail = async () => {
		if (user) {
			try {
				await client.users.requestPasswordReset(user.email)
	...
	return (
		<Paper p='md' mt='md'>
			...
			<form onSubmit={onSubmit(handleSubmit)} style={{ position: 'relative' }}>
				<TextInput
					label='Mã'
					placeholder='Mã xác nhận đổi mật khẩu ở đây'
					mt='md'
					{...getInputProps('token')}
				/>
				<PasswordInput
